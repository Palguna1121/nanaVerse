...
<div class="w-full">
    <div class="flex items-center justify-between mb-8">
        ...
        {#if $cartCount > 0}
        <button onclick={() => showClearModal = true} class="p-2 text-muted hover:text-red-500 hover:bg-red-50 rounded-full transition-all duration-200" title="Clear Cart">
    ...
    <div class="fixed inset-0 z-100 flex items-center justify-center">
        ...
        <div 
            ...
            <h3 class="text-xl font-heading font-bold text-center text-main mb-2">Clear Cart?</h3>
            <p class="text-sm font-body text-muted text-center mb-8">All items in your cart will be permanently removed. Are you sure?</p>
            
            <div class="flex gap-4">
                <button onclick={() => showClearModal = false} type="button" class="flex-1 px-4 py-3 bg-card-elevated text-main font-semibold rounded-full hover:bg-border-light transition-colors">Cancel</button>
                <button onclick={handleClearCart} class="flex-1 px-4 py-3 bg-red-600 text-white font-semibold rounded-full hover:bg-red-700 transition-colors">Yes, Clear</button>
            </div>
        </div>
    </div>
    {/if}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <li class="p-6 flex items-center group">
                        ...
                        <div class="ml-6 flex flex-1 flex-col">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-muted font-body font-normal leading-[1.7]">Price per unit: {formatPrice(item.price)}</p>
                            </div>
                            <div class="flex flex-1 items-end justify-between text-sm mt-4">
                                <div class="flex items-center border border-border-light rounded-full p-1 bg-card-elevated/50">
                                    <button onclick={() => updateQuantity(item.id, 'decrease')} class="w-7 h-7 flex items-center justify-center rounded-full bg-card text-main shadow-sm border border-border-light hover:bg-card-elevated transition-colors" title="Decrease quantity">
                                    ...
                                    <button onclick={() => updateQuantity(item.id, 'increase')} class="w-7 h-7 flex items-center justify-center rounded-full bg-brand text-white shadow-sm hover:bg-brand-hover transition-colors" title="Increase quantity">
        ...
        <div class="lg:col-span-1">
            <div class="bg-card shadow-soft border border-border-light rounded-[24px] p-6 sticky top-28">
                <h2 class="text-lg font-heading font-bold text-main mb-5 tracking-tight leading-[1.2]">Order Summary</h2>
                <div class="space-y-3 font-body font-normal">
                    ...
                    <div class="border-t border-border-light pt-5 mt-5 flex justify-between items-center">
                        <p class="text-base font-heading font-bold text-main">Total Bill</p>
                ...
                <div class="mt-6">
                    <div class="bg-nav-active rounded-[12px] p-3 mb-5 border border-brand/10">
                        ...
                        <p class="text-[11px] text-brand/80 font-body font-normal leading-[1.6]">You will be redirected to the secure Xepeng payment simulator.</p>
                    </div>
    ...
    <div class="bg-card border border-border-light shadow-soft rounded-[32px] p-20 text-center max-w-3xl mx-auto mt-6 min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h2 class="text-3xl font-heading font-bold text-main mb-4 tracking-tight leading-[1.2]">Your Cart is Empty</h2>
            <p class="text-lg text-muted font-body font-normal mb-10 max-w-md mx-auto leading-[1.7]">Start exploring our premium collection. Find your favorite items and enjoy the best shopping experience.</p>
